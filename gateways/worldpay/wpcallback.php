<?php
		...
		if ( (isset($_REQUEST["transId"]) && $_REQUEST["transStatus"]=='Y') || (isset($_REQUEST["futurePayId"]) && $_REQUEST["transStatus"]=='Y') ) :
			...
			$order 				  = addslashes( $_REQUEST["CM_order"] );
			...
			$url 				  = addslashes( $_REQUEST["CM_SuccessURL"] );
			...
			if ( !$MC_transactionNumber || $MC_transactionNumber == '' ) :
				 ...
				 */
				$worldpay_args 		= array();
				foreach($worldpay_args_array as $param => $value) $worldpay_args[] = "$param=$value";
				...
				$url 				= $subscriptionurl .'&order='. $order;

				if ( $woocommerce_worldpay_settings['worldpaydebug'] == 'yes' ) :
					wp_mail( $woocommerce_worldpay_settings['worldpaydebugemail'], 'WorldPay CallBack Details ' . $order, mailcontent( $worldpay_args_array ) . ' ' . $url );
				endif;
